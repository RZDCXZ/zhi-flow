...
Zhi Flow 是一个按里程碑推进的单用户 AI 聊天与 RAG 学习项目。当前完成到里程碑 05：浏览器可以管理 Conversation，通过版本化 SSE 观察回答增量，并在刷新后恢复已持久化的 Message。用户 Message 与 `streaming` 助手尝试在模型调用前原子创建；完成、取消、失败和遗留中断都有明确终态。多轮上下文、重生成、RAG 与供应商切换 UI 尚未实现。
...
flowchart LR
  ...
  Route --> Persist["原子创建用户 Message<br/>与 streaming 助手 Message"]
  Persist --> Provider["Chat Provider 合约"]
...
`FakeChatProvider` 通过相同的 `ChatProvider.stream({ message, signal })` 合约返回可控增量并记录收到的请求，适合观察消息结构、取消和用量映射。自动测试还会启动本地假 OpenAI-compatible HTTP 上游，以覆盖真实适配器的请求体、Authorization 头、超时与错误映射；它不会访问真实模型。
...
- `src/app/api/chat/route.ts` 在流开始前创建 Message 对，边流边持久化正文，并在完成、取消或失败时写入终态。
- `src/server/conversations.ts` 集中 Conversation/Message 的数据库操作；`create_message_attempt` RPC 保证用户 Message 与助手尝试原子创建并处理幂等重放。
